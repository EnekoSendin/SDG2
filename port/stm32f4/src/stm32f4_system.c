...
/**
 *@brief Lee el valor de un pin (IDR)
 */

bool stm32f4_system_gpio_read(GPIO_TypeDef *p_port, uint8_t pin)
{
	uint32_t base_mask = BIT_POS_TO_MASK(pin);
	bool value = (bool)(p_port->IDR & base_mask);
	return value;
}

/**
 *@brief Escribe el valor de un pin (BSRR)
 */
...
/**
 *@brief Invierte el valor del pin
 */
